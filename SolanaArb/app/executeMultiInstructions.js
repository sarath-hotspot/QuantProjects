...
const executeMultiInstructions = async () => {
    ...
    // Execute increment and increment by in single transaction. 
    let txn = await program.methods.increment()
        .accounts({
            baseAccount: baseAccount.publicKey,
        })
        .signers([walletKeyPair])
        .transaction();
    let instr2 = await program.methods.incrementBy(new BN(100)).accounts(
        {
            baseAccount: baseAccount.publicKey,
        })
        .signers([walletKeyPair])
    ...
    let txnSignature = await solanaWeb3.sendAndConfirmTransaction(connection, txn, [walletKeyPair]);
